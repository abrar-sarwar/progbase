...
app/
  ...
  _actions/members.ts            updateMember, blockMember server actions (shared)
  ...
  blacklist/actions.ts           addBlacklist, removeBlacklist
components/
...
// NOTE: `nullsFirst` was added to @supabase/supabase-js order() options in
// v2.36+. If `npx tsc --noEmit` reports that `nullsFirst` is not a valid
// option on the installed version, drop that key:
//     .order("first_seen", { ascending: true });
// Default behavior puts nulls last in ascending order, which is acceptable
// for the members list (recently-added members with missing first_seen sort
// to the bottom as a tiebreaker).
...
`blockMember` is referenced but defined in a later task. Add a stub so this task compiles standalone. The file lives at `app/_actions/members.ts` (a prefixed `_actions/` directory that Next.js excludes from routing) so both the members-table and the member-edit-form can import it without cross-route server-action edge cases:
...
```bash
git add components/members-table.tsx app/_actions/members.ts
...
export async function addBlacklist(input: {
  ...
  // Hide any matching member rows. Log one member_edits row per member that
  // transitions from hidden=false → hidden=true so the audit trail matches
  // the block action in app/_actions/members.ts.
...
- `isAllowed` signature `(email, rawAllowlist)` consistent across `lib/allowlist.ts`, `middleware.ts`, `app/_actions/members.ts`, `app/blacklist/actions.ts`, `app/import/actions.ts`.
